...
ms_clip <- function(target, clip = NULL, bbox = NULL, remove_slivers = FALSE,
                    force_FC = TRUE, sys = FALSE, sys_gb = 8) {
...
ms_clip.character <- function(target, clip = NULL, bbox = NULL,
                              remove_slivers = FALSE, force_FC = TRUE, sys = FALSE, sys_gb = 8) {
  target <- check_character_input(target)

  clip_erase_json(target = target, overlay_layer = clip, type = "clip",
                  remove_slivers = remove_slivers, bbox = bbox, force_FC = force_FC, sys = sys, sys_gb = sys_gb)
...
clip_erase_geo_list <- function(target, overlay_layer, bbox, type,
                                remove_slivers, force_FC, sys, sys_gb) {
...
clip_erase_sp <- function(target, overlay_layer, bbox, type, remove_slivers, force_FC, sys, sys_gb) {
  ...
  result <- mapshaper_clip_erase(target_layer = target_geojson,
                                 ...
                                 bbox = bbox, force_FC = TRUE, sys = sys, sys_gb = sys_gb)
...
clip_erase_sf <- function(target, overlay_layer, bbox, type, remove_slivers, force_FC, sys, sys_gb) {
...
mapshaper_clip_erase <- function(target_layer, overlay_layer, bbox, type,
  ...
  if (!is.null(bbox)) {
    cmd <- paste0("-", type, " bbox=",paste0(bbox, collapse = ","), " ",
                  remove_slivers)
    out <- apply_mapshaper_commands(target_layer, cmd, force_FC = force_FC, sys = sys, sys_gb = sys_gb)
  } else if (!is.null(overlay_layer)) {

    if (sys) {
      on.exit(unlink(c(target_layer, overlay_layer)), add = TRUE)
      cmd <- paste0("-", type)
      out <- sys_mapshaper(data = target_layer, data2 = overlay_layer, command = cmd, sys_gb = sys_gb)
    } else {
